...
if page == pages[5] : 
  ...
  st.write("The best-performing model was the **Random Forest Regression**, with **R² values of 0.242** for the test set, and the **lowest MAE (29.177)** and **RMSE (36.320)**. This model provided the most accurate pricing predictions, balancing accuracy and error minimization. While regression models like Random Forest provided the best results for predicting continuous price values, the **Random Forest Classifier** showed **88.2% accuracy** for classification tasks and solid performance across all categories.")
